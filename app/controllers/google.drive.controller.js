...
exports.onAdd = async (req, res) => {
    try {
        let userEmail = jwt.verify(req.body.jwtToken, req.JWT_SECRET).email;
        if (!req.body || !req.body.refreshToken)
            throw new Error();

        let user = await req.db.users.findOne({where: {email: userEmail}});
        let drive = await req.db.googleDrives.create({refreshToken: req.body.refreshToken});
