 ...
 * =================
 */
const ONEDRIVE_TOKEN_GRANTING_URL =
...
const ONEDRIVE_SCOPE =
    "Files.ReadWrite.All Files.ReadWrite.AppFolder Files.ReadWrite.Selected offline_access";
    ...
    res.writeHead(StatusCodes.TEMPORARY_REDIRECT, {
        Location:
            ...
            (process.env.UNST_IS_SERVER_UP
                ? `${ONEDRIVE_REMOTE_REDIRECT}/onedrive/get_token`
                : `${ONEDRIVE_LOCAL_REDIRECT}/onedrive/get_token`) +
...
async function onDriveAddition(req, res) {
    try {
        ...
        if (!req.body || !req.body.refreshToken) {
            throw new Error(
                "Cannot add OneDrive because the request is invalid.",
            );
        }
...
async function getTokensHavingCode(req, res) {
    ...
    await fetch(
        process.env.UNST_IS_SERVER_UP
            ? `${ONEDRIVE_REMOTE_REDIRECT}/onedrive/add`
            : `${ONEDRIVE_LOCAL_REDIRECT}/onedrive/add`,
        {
            method: "POST",
            body: JSON.stringify({
